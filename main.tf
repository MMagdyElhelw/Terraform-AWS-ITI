...
module "Project_backend" {
  source  = "./Modules/Backend.md"
  S3_Name = "project-magdy-aswan"
}
module "key-pair" {
  source       = "./Modules/SSH.md"
  key-name     = "Project"
...
module "BE1-Apache" {
  ...
  user-data         = <<-EOF
     ...
     echo "<h1><center> Backend Server 1 </center></h1>" > /var/www/html/index.html
...
module "BE2-Apache" {
  ...
  user-data         = <<-EOF
     ...
     yum install -y httpd
     systemctl enable --now httpd
     echo "<h1><center> Backend Server 2 </center></h1>" > /var/www/html/index.html
     systemctl restart httpd
   EOF
}
